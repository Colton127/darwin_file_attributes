...
public class DarwinFileAttributesPlugin: NSObject, FlutterPlugin {
        ...
        for (key, value) in values {
            ...
            case "fileProtection":
                if let str = value as? String,
                   let prot = Self.nsFileProtectionFromString(str) {
                    do {
                        try FileManager.default.setAttributes(
                            [.protectionKey: prot], ofItemAtPath: path
                        )
                    } catch {
                        result(FlutterError(code: "RESOURCE_ERROR",
                                            message: error.localizedDescription,
    ...
    private static func nsFileProtectionFromString(_ s: String) -> FileProtectionType? {
        switch s {
        case "none":                                return .none
        case "complete":                            return .complete
        case "completeUnlessOpen":                  return .completeUnlessOpen
